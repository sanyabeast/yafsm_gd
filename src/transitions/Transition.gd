...
signal use_target_as_trigger_changed(enabled)
...
@export var use_target_as_trigger: = false:
...
func transit(params={}, local_params={}):
	if use_target_as_trigger:
		var has_target_trigger = params.get(to) == null and params.has(to)
		var has_local_target_trigger = local_params.get(to) == null and local_params.has(to)
...
func set_use_target_as_trigger(value: bool):
